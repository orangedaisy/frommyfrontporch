...
task :watch, :option do |t, args|
  option = args[:option]
  if option.nil? or option.empty?
    execute("jekyll serve --config _config.yml,_config-dev.yml --watch")
  else
    if option == "drafts"
      execute("jekyll serve --config _config.yml,_config-dev.yml --watch --drafts")
    else
      execute("jekyll serve --config _config.yml,_config-dev.yml --watch --limit_posts #{option}")
    end
  end
end
